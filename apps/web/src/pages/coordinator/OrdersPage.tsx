...
import { Plus, Clock, Truck, CheckCircle, XCircle, Package, AlertTriangle } from 'lucide-react'
...
const TABS: { key: OrderStatus | 'all'; label: string; icon: React.ReactNode; color: string }[] = [
  ...
  { key: 'assigned',   label: 'Đã gán',    icon: <Truck size={13} />,         color: '#2563eb' },
      ...
      {showCreate && (
        <>
          <div
            ...
            style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.3)', zIndex: 40 }}
          />
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          {order.assigned_driver && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end', marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 11, color: '#475569', fontWeight: 500 }}>{order.assigned_driver.full_name}</span>
            </div>
          )}
